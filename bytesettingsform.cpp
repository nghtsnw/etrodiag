...
void ByteSettingsForm::open(int _devNum, int _byteNum)
{
    ...
    else if ((40 - byteNum) >= 2) {
        ui->bitBox->setMaximum(16);
    }
    else if ((40 - byteNum) >= 0) {
        ui->bitBox->setMaximum(8);
    }
...
void ByteSettingsForm::addMaskItem(s_parameterMask mask)
{
    if (devNum == mask.devNum && byteNum == mask.byteNum)
    {
        ...
        if (ui->masksWidget->rowCount() > 0)
        {
            ...
            {
                if (QString::number(mask.id, 10) == ui->masksWidget->item(i, 2)->text()) {
                    findRow = true;
                }
            }
        }
...
{ //возврат значения длины слова из bytedefinition в ответ на запрос из bytesettingsform
    ...
    {
        if (wordType == 0) {
            this->ui->bitBox->setValue(8);
        }
        else if (wordType == 1) {
            this->ui->bitBox->setValue(16);
        }
        else if (wordType == 2) {
            this->ui->bitBox->setValue(32);
        }
    }
}
...
void ByteSettingsForm::updateMasksList(s_parameterMask mask)
{
    ...
    {
        for (int i = 0; i < ui->masksWidget->rowCount(); i++) {
            if (ui->masksWidget->item(i, 2)->text() == QString::number(mask.id, 10))
            {
                ui->masksWidget->item(i, 0)->setText(mask.parameterName);
                QString endValue2String;
                endValue2String.setNum(mask.endValue);
